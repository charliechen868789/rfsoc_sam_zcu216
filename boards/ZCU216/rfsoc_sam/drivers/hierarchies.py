            ...
            for j in range(0, NUM_ADC_BLOCKS):
                ...
                [''.join(['C_ADC_Slice', str(i), str(j*2), '_Enable'])] == 'true':
                    ...
                    [''.join(['C_ADC', str(i), '_Enable'])] == '1':
                        ...
                        # ZCU216 fix: index by the real HW block number (j*2, matching
                        # the C_ADC_Slice{i}{j*2}_Enable check above), not the raw loop
                        # index j. This was latent on ZCU208's ZU48DR (whose I/Q-pairing
                        # made block 1 implicitly valid too) but errors on ZU49DR, which
                        # has no such implicit pairing: block 1 genuinely isn't enabled.
                        adc_blocks.append(getattr(self, rfdc_core).adc_tiles[i].blocks[j*2])
...
class Receiver(DefaultHierarchy):
    ...
    def _initialise_adc_tile(self, tile):
        # ZCU216 (ZU49DR) port: tile.DynamicPLLConfig() is broken in this
        # xrfdc driver build for this device -- it rejects even the tile's
        # own already-correct compiled values (confirmed on real hardware:
        # DynamicPLLConfig(1, 409.6, 2.4576) on ADC0 raises "Invalid
        # sampling rate value" even though that is exactly what the
        # bitstream's RFDC IP was synthesised with). The call is redundant
        # here anyway -- PLL_FREQUENCY/MAX_SAMPLE_FREQUENCY in constants.py
        # already match what was configured in Vivado at synthesis time, so
        # ShutDown()+StartUp() alone (which does work) is sufficient.
        tile.ShutDown()
        sleep(1)
        tile.StartUp()
...
class Transmitter(DefaultHierarchy):
    ...
    def _initialise_dac_tile(self, tile):
        # See Receiver._initialise_adc_tile: same broken-DynamicPLLConfig
        # workaround, on the transmit side.
        tile.ShutDown()
        sleep(1)
        tile.StartUp()
